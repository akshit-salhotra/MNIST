...
class IsotropicGaussian(nn.Module, Sampleable):
    ...
    def __init__(self, shape: List[int], std: float = 1.0):
        ...
        self.register_buffer("dummy", torch.zeros(1))# this is equivalent , its just that nn.Buffer was introduced in torch verison 2.7
        
        # when a Buffer object is assigned to nn.Module attribute -> it becomes a part of the module state (but
        # not a parmater ). Hence , whenever object is moved to a certain device , this buffer automatically , moves to the same device.
